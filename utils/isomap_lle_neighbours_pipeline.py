"""
==========
ISOMAP && LLE neighbours parameter CV pipeline
==========

Use a pipeline to find the best neighbourhood size parameter for LLE & ISOMAP.
...
op.add_option("--method",
              dest="method", help="use this algorithm for neighbour checking")
...
m_dict = {'isomap': Isomap(n_neighbors=5, n_components=30), 'lle': LocallyLinearEmbedding(n_neighbors=5, n_components=30)}
...
method_obj = m_dict[method_name]

pipe = Pipeline(steps=[(method_name, method_obj), ('kmeans', kmeans)])

# Set the model parameters to cycle over using '__' a prefix
estimator = GridSearchCV(pipe, dict(method_obj__n_neighbors=neighbours), scoring=homogeneity_scorer, n_jobs=opts.jobs)
