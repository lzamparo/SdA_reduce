...
curtailed_data <- subset(both_layers, epoch <= 50 & method != "NAG")

curtailed_p <- ggplot(curtailed_data, aes(epoch,score, colour = method))
curtailed_p <- curtailed_p + geom_smooth(alpha = 1/3)
curtailed_p <- curtailed_p + scale_x_discrete(limits = c(1,50), breaks = seq(0,50,by=10))
curtailed_p <- curtailed_p + scale_y_continuous(limits = c(100,400))
...
curtailed_p <- curtailed_p + theme(axis.title.x = element_text(size = 15))
curtailed_p <- curtailed_p + theme(axis.text.y = element_text(size = 13, face = "bold"))
curtailed_p <- curtailed_p + theme(axis.title.y = element_text(size = 15))
