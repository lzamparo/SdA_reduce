...
cd $PBS_O_WORKDIR
python hybrid_pretrain_SdA_multiproc.py -d "${SCRATCH}/gpu_models/gb_hybrid_cm/4_layers/pretrain_output/$OUTDIRSUFFIX" -c 0.20 -a $FIRSTMODEL -b $SECONDMODEL -i "${SCRATCH}/sm_rep1_data/sm_rep1_screen.h5" -o $OFFSET -t $LAYERTYPES -l $LOSS -m NAG -s 15
